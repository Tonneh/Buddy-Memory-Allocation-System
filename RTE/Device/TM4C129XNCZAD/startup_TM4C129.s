...
Reset_Handler   PROC
                ...
                IMPORT  SystemInit
                IMPORT  __main
				IMPORT  _systemcall_table_init 			; 	import systemcall_table_init function 
				IMPORT  _heap_init 						; 	import heap_init function 
														;	set up MSP, do not change the following three lines
				
				LDR     R0, =__initial_sp 				; 	thread mode uses MSP
				MSR     MSP, R0 						; 	write the contents of r0 into main stack pointer 
				ISB  									;	Instruction Synchronization Barrier, flushes the pipeline of the processor.
														;	load the addresses of the functions into r0 then branch them one by one 
                LDR     R0, =SystemInit					; 	load address of systeminit into R0 
                BLX     R0								; 	branch to systeminit 
				LDR     R0, =_heap_init 				;	load address of heap_init into r0 
				BLX     R0								; 	branch to heap_init 
				LDR     R0, =_systemcall_table_init 	; 	load address of systemcall_table_init into r0 
				BLX     R0								; 	branch to systemcall_table_init 
				
														;	set up thread stack, do not change any code from here to ENDP
				LDR     R0, =__initial_user_sp 			; 	load the initial user stack poitner into r0 
				MSR     PSP, R0 						;	write contents of r0 into process stack pointer 
														; 	change BIT1 of CONTROL register to enable the PSP and MSP setting
				MOVS    R0, #2							;	Set SPSEL bit: the PSP is used in thread mode and MSP is used in handler mode ; see book section 2.5
				MSR     CONTROL, R0						;	Now thread mode uses PSP for user
                LDR     R0, =__main						; 	load addres of min into r0 
...
SVC_Handler     PROC
				...
				PUSH 	{lr}					; need to push LR since we'll be coming back here afterwards 
												; add codes to call _systemcall_table_jump
				LDR R1, =_systemcall_table_jump	
				BLX R1 
												; do not update the following two lines
                MRS	R1, PSP						; write contents of PSP into R1 
				STR	R0, [R1]					; store r0 into r1 
												; add one line of code here to where SVC_hanlder is called (_malloc or _free)
				POP {lr}						; pop the lr 
				BX	lr							; branch to lr 
                ENDP
DebugMon_Handler\
